...
from config import torch, num_epochs, device, testloader
...
def main():
    ...
    # Save model
    checkpoint_dir = "/home/wchung25/eee515/HW2/EEE515_HW2/checkpoint_cnn"
    
    # Ensure the directory exists
    if not os.path.exists(checkpoint_dir):
        os.makedirs(checkpoint_dir, exist_ok=True)

    checkpoint_path = os.path.join(checkpoint_dir, "cnn_model_checkpoint.pth")
    
    # Save plot
    plot_filename = os.path.join(checkpoint_dir, "lesnetOG_training_validation_loss_plot.png")
    plot_losses(training_losses, validation_losses, plot_filename)
    
    accuracy_plot_filename = os.path.join(checkpoint_dir, "lesnetOG_training_validation_accuracy_plot.png")
