...
class CIFAR10S(torchvision.datasets.CIFAR10):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False, skew_ratio=0.95, all_grayscale=False, train_grayscale=False):
    ...
    def _skew_dataset(self):
        ...
        if self.all_grayscale:
            # Convert the whole dataset to grayscale
            self._convert_to_grayscale(range(len(self.data)))
        elif self.train and self.train_grayscale:
            # Convert only the training dataset to grayscale
...
trainset = CIFAR10S(root='./data', train=True, download=True, transform=augmented_transform, train_grayscale=True)
